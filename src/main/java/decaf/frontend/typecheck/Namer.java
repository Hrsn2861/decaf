...
public class Namer extends Phase<Tree.TopLevel, Tree.TopLevel> implements TypeLitVisited {
    ...
    public void visitClassDef(Tree.ClassDef clazz, ScopeStack ctx) {
        ...
        clazz.symbol.isAbstract = clazz.isAbstract();

        if (clazz.hasParent()) {
            clazz.superClass.accept(this, ctx);
            var parent = clazz.symbol.parentSymbol.get();
            clazz.symbol.abstractMethod = (HashSet<String>)parent.abstractMethod.clone();
        }
        ...
        if (!clazz.symbol.abstractMethod.isEmpty() && !clazz.isAbstract()) {
            issue(new AbstractError(clazz.pos, clazz.name));
            ctx.close();
            clazz.resolved = true;
            return;
        }
    ...
    public void visitMethodDef(Tree.MethodDef method, ScopeStack ctx) {
        ...
        if (earlier.isPresent()) {
            var bufSymbol = (MethodSymbol)earlier.get();
            if (earlier.get().isMethodSymbol() && (!method.isAbstract() || bufSymbol.isAbstract())) { // may be overriden
                ...
                if (suspect.domain() != ctx.currentScope() && !suspect.isStatic() && !method.isStatic()) {
                    ...
                    if (method.type.subtypeOf(suspect.type)) { // override success
                        ...
                        var parentSymbol = method.symbol.owner;
                        var buflist = parentSymbol.abstractMethod;

                        if (buflist.contains(name) && !method.isAbstract()) {
                            // 如果含有
                            buflist.remove(name);
                        }
